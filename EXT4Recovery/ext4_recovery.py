...
Module chính để phục hồi dữ liệu EXT4 khi có tham số volume sai
Xử lý các trường hợp: superblock hỏng, group descriptor lỗi, inode bị mất
Tương tự như Volume class trong volume.cpp
"""
...
class EXT4Recovery:
    """
    Class chính để phục hồi dữ liệu từ EXT4 filesystem
    """
    
    def __init__(self, device_path: str = None):
        """
        Khởi tạo EXT4Recovery
        
        Args:
            device_path: Đường dẫn đến device/image file
        """
    ...
    def open_device(self, device_path: str) -> bool:
        """
        Mở device/image file để phân tích
        Tương tự readVolume() trong volume.cpp
        
        Args:
            device_path: Đường dẫn đến device/image
        ...
        if self.read_primary_superblock():
            ...
            print("⚠️  Superblock chính bị hỏng, đang tìm backup...")
            return self.find_backup_superblocks()
    
    def read_primary_superblock(self) -> bool:
        """
        Đọc superblock chính (offset 1024)
        
        Returns:
            True nếu thành công và superblock hợp lệ
        """
    ...
    def find_backup_superblocks(self) -> bool:
        """
        Tìm các backup superblocks
        EXT4 lưu backup superblocks tại các block groups: 0, 1, và các lũy thừa của 3, 5, 7
        
        Returns:
            True nếu tìm thấy ít nhất một backup hợp lệ
        """
        ...
        for block_size in possible_block_sizes:
            ...
            for base in [3, 5, 7]:
                power = base
                while power < 1000:  # Giới hạn tìm kiếm
                    backup_groups.append(power)
                    power *= base
            
            backup_groups.sort()
            
            for group_num in backup_groups[:20]:  # Kiểm tra 20 groups đầu tiên
                # Tính offset của superblock trong group này
                if block_size == 1024:
                    # Với block size 1024, superblock luôn ở offset 1024
                    offset = group_num * block_size * 8192 + 1024
                else:
                    # Với block size lớn hơn, superblock ở block đầu tiên
    ...
    def print_superblock_info(self):
        """
        In thông tin superblock
        Tương tự printSuperBlock() trong volume.cpp
        """
        if not self.superblock:
    ...
    def read_inode(self, inode_number: int) -> Optional[Inode]:
        """
        Đọc một inode cụ thể
        
        Args:
            inode_number: Số inode cần đọc
    ...
    def list_directory(self, inode_number: int = 2) -> List[DirectoryEntry]:
        """
        Liệt kê nội dung của directory
        
        Args:
            inode_number: Số inode của directory (mặc định là 2 - root)
            
        Returns:
            Danh sách các directory entries
        """
    ...
    def _list_directory_blocks(self, inode: Inode) -> List[DirectoryEntry]:
        """
        Liệt kê directory sử dụng block pointers
        """
        entries = []
    ...
    def _list_directory_extents(self, inode: Inode) -> List[DirectoryEntry]:
        """
        Liệt kê directory sử dụng extent tree
        """
    ...
    def _parse_directory_entries(self, data: bytes) -> List[DirectoryEntry]:
        """
        Parse directory entries từ block data
        """
        entries = []
        offset = 0
    ...
    def scan_for_inodes(self, start_block: int = 0, num_blocks: int = 100) -> List[int]:
        """
        Quét tìm các inodes còn sống trong khoảng blocks
        Hữu ích khi group descriptor bị hỏng
        
        Args:
            start_block: Block bắt đầu quét
            num_blocks: Số blocks cần quét
            
        Returns:
            Danh sách inode numbers tìm được
        """
    ...
    def recover_file(self, inode_number: int, output_path: str) -> bool:
        """
        Phục hồi file từ inode
        Tương tự exportFile() trong volume.cpp
        
        Args:
            inode_number: Số inode của file cần phục hồi
        ...
        try:
            with open(output_path, 'wb') as f:
                ...
                if inode.i_flags & EXT4_EXTENTS_FL:
                    ...
                    print("⚠️  File sử dụng extents (chưa hỗ trợ đầy đủ)")
                    return False
                else:
